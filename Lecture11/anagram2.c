...
void scan_word(int* occurrences) {
    ...
    while ((c = getchar()) != '\n') {                               // read characters until a newline
        if (isdigit(c)) {                                           // to check if the input is a digit
            ...
            return scan_word(occurrences);                          // ask for user input again
        } else if (isalpha(c)) {                                    // to check if it is a letter
            occurrences[toupper(c) - 'A']++;                        // count increment
        }
    }
}
...
bool is_anagram(int* occurrences1, int* occurrences2) {
    for (int i = 0; i < 26; i++) {
        if (occurrences1[i] != occurrences2[i]) {                   // compare the two occurences
            return false;                                           // not equal means not an anagram
        }
    }
    return true;                                                    // anagram if all counts match
}
